...
int getMax(int *st, int s, int e, int l, int r, int si) {
    if(s>=l && e<=r) {
        return st[si];
    }
    else if(s>r || e<l)
        ...
        int mid=getMid(s,e);
        return maxVal(getMax(st,s,mid,l,r,si*2+1),getMax(st,mid+1,e,l,r,si*2+2));
    }
}
...
int fill_st(int a[],int s, int e, int* st, int si) {
    ...
    st[si]=maxVal(fill_st(a,s,mid,st,si*2+1),fill_st(a,mid+1,e,st,si*2+2));
    return st[si];
}
